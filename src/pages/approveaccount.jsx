...
export const sendEmail = async (toEmail, templateId) => {
  try {
    const templateParams = {
      to_email: toEmail,
      firestore_api_link: 'https://firestore.googleapis.com/v1/projects/banking-8d939/databases/(default)/documents/customerAccounts',
      // Add other template parameters here as needed
    };
 
    const serviceId = 'service_5u97f7c'; // Replace with your EmailJS service ID
    const userId = 'fCLEqIS-I5wLgNRI6'; // Replace with your EmailJS user ID
...
function ApproveAccountsPage() {
  ...
  const fetchPendingRequests = async () => {
    try {
      const response = await axios.get('https://firestore.googleapis.com/v1/projects/banking-8d939/databases/(default)/documents/customerAccounts');
  ...
  const updateRequestStatus = async (requestId, newStatus, email, phoneNumber) => {
    try {
      ...
      await axios.patch(`https://firestore.googleapis.com/v1/projects/banking-8d939/databases/(default)/documents/customerAccounts/${requestId}`, {
        fields: updatedFields
      });
 
      // Send email regardless of status
      const templateId = newStatus === 'approved' ? 'template_ouyxzt4' : 'template_0s7o87e'; // Use different template for rejection
      ...
      if (newStatus === 'approved') {
        ...
        await axios.post('https://firestore.googleapis.com/v1/projects/banking-8d939/databases/(default)/documents/Account', {
          fields: {
            accountNumber: { integerValue: accountNumber },
  ...
  return (
    ...
    {/* <div style={{ fontFamily: 'Arial, sans-serif', margin: '0 auto', padding: '20px',width:'500px',height:'400px' }}> */}
      <h1 style={{ color: '#fff', textAlign: 'center', marginBottom: '20px' }}>Approve Accounts</h1>
      {error && <p style={{ color: 'red', textAlign: 'center' }}>{error}</p>}
      {pendingRequests.length === 0 ? (
        <p style={{ textAlign: 'center', fontStyle: 'italic', color:'white' }}>No pending account requests</p>
      ) : (
