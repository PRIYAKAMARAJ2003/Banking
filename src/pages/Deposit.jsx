...
function Deposit() {
  ...
  const handleDeposit = () => {
    ...
    fetch(`https://firestore.googleapis.com/v1/projects/banking-8d939/databases/(default)/documents/Account`)
      .then(response => {
        if (!response.ok) {
      ...
      .then(data => {
        ...
        return fetch(`https://firestore.googleapis.com/v1/projects/banking-8d939/databases/(default)/documents/Account/${docId}`, {
          method: 'PATCH',
          headers: {
      ...
      .then(response => {
        ...
        return fetch(`https://firestore.googleapis.com/v1/projects/banking-8d939/databases/(default)/documents/Transactions`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
          },
    ...
    <div className="container1" style={{
      ...
      <div className="form">
        ...
        <div>
          ...
          <input type="text" value={accountNumber} readOnly style={{width:'200px', height:'30px'}}/>
        </div>
        <div>
          <label>Name:</label>
          <input type="text" value={name} onChange={e => setName(e.target.value)} style={{width:'200px', height:'30px'}} />
        </div>
        <div>
          <label>Amount:</label>
          <input type="text" value={amount} onChange={e => setAmount(e.target.value)} style={{width:'200px', height:'30px'}}/>
        </div>
        <button onClick={handleDeposit} style={{backgroundColor:'#073d39'}}>Deposit</button>
        {message && <p>{message}</p>}
      </div>
    </div>
  );
