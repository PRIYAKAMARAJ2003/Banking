...
function LoanDetails() {
  ...
  const fetchPendingRequests = async () => {
    try {
      const response = await axios.get('https://firestore.googleapis.com/v1/projects/banking-8d939/databases/(default)/documents/Loan');
    ...
    try {
      if (status === 'approved') {
        ...
        if (sanctionedAmount !== null) {
          const accountsResponse = await axios.get('https://firestore.googleapis.com/v1/projects/banking-8d939/databases/(default)/documents/customerAccounts');
          const accounts = accountsResponse.data.documents;
 
          const accountResponse = await axios.get('https://firestore.googleapis.com/v1/projects/banking-8d939/databases/(default)/documents/Account');
          ...
          if (accountToUpdate) {
            ...
            await axios.patch(`https://firestore.googleapis.com/v1/projects/banking-8d939/databases/(default)/documents/customerAccounts/${accountToUpdate.name.split('/').pop()}`, {
              fields: {
            ...
            await axios.patch(`https://firestore.googleapis.com/v1/projects/banking-8d939/databases/(default)/documents/Account/${accToUpdate.name.split('/').pop()}`, {
              fields: {
      ...
      await axios.patch(`https://firestore.googleapis.com/v1/projects/banking-8d939/databases/(default)/documents/Loan/${id}`, {
        fields: {
  ...
  return (
    ...
    {/* <div style={{ fontFamily: 'Arial, sans-serif', maxWidth: '800px', margin: '0 auto', padding: '20px' }}> */}
      <h1 style={{ color: '#fff', textAlign: 'center', marginBottom: '20px' }}>Approve/Reject Loans</h1>
      {error && <p style={{ color: 'red', textAlign: 'center' }}>{error}</p>}
      {pendingRequests.length === 0 ? (
        <p style={{ textAlign: 'center', fontStyle: 'italic',color:'white' }}>No pending loan requests</p>
      ) : (
