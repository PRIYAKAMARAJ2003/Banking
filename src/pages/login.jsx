...
const LoginPage = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`https://firestore.googleapis.com/v1/projects/banking-8d939/databases/(default)/documents/customerAccounts/${trimmedCustomerId}`);
      if (!response.ok) {
  ...
  return (
    <div className='Login'>
      ...
      <form onSubmit={handleLogin}> {/* Use onSubmit to handle form submission */}
        ...
        />
        <button type="submit" style={{backgroundColor:'gray'}}>Login</button> {/* Use type="submit" for the login button */}
      </form>
      {error && <p>{error}</p>}
    </div>
  );
